from GeoAutoViz.interfaces.DataSource import DataSource, Offline, Mode
...
class UrbanFeaturesDataSourceOfflineDataSource(DataSource):
    ...
    def extract_tessellation(self, source=Mode.Online, buffered_limit=100, save_offline=False, mode=Offline.DB):
        if source == Mode.Online:
    ...
    def extract_streets(self, source=Mode.Online, network_type='drive', save_offline=False, mode=Offline.DB):
        if source == Mode.Online:
    ...
    def extract_data(self, source=Mode.Online, save_offline=False, mode=Offline.DB):
